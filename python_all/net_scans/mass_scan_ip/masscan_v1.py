...
def run_masscan(rate, subnet, mask, ports):
    ...
    Args:
        ...
        subnet (str): Подсеть для сканирования (например, 192.168.1.0)
        mask (str): Маска подсети (например, 24)
        ports (str): Диапазон портов (например, 1-100 или пустой для всех)
    """
    try:
        # Проверяем, установлен ли masscan
        ...
        error_msg = f"Ошибка при выполнении masscan: {e}\n"
        print(error_msg)
        with open('report_massscan.txt', 'w') as report:
            report.write(error_msg)
...
def validate_args(args):
    ...
    if len(args) != 5:
        ...
        print("Пример: python script.py 5 192.168.1.0 24 1-100")
        print("Для всех портов: python script.py 5 192.168.1.0 24 -p-")
        return False
...
if __name__ == "__main__":
    if validate_args(sys.argv):
        ...
        mask = sys.argv[3]
        ports = sys.argv[4]
        # Преобразуем -p- в пустую строку для внутренней логики
